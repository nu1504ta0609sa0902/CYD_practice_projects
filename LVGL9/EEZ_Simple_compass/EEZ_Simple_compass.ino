...
int maxValue = random(0, 361);
int updateCompassValueEvery = random(0, 10);

void mockCompassDirection() {
  ...
  if (directionChanged) {
    maxValue = random(0, 361);    
    updateCompassValueEvery = random(0, 10); 
  }
}
...
void loop() {
  ...
  mockCompassDirection();
  if (value % updateCompassValueEvery == 0) {

    lv_image_set_rotation(objects.compass_image, value * 10);
    lv_label_set_text_fmt(objects.compass_value1, "%d", value);
