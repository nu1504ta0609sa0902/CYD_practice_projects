...
void setup() {
  ...
  lv_obj_add_event_cb(objects.arc_shadow_bg, value_changed_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
}
...
void mockData() {
  //Add some randomness
  int change = random(1, maxRandomValue);
...
static void value_changed_event_cb(lv_event_t *e) {
  ...
  //LV_LOG_USER("ARC value changed");

  if (code == LV_EVENT_VALUE_CHANGED) {
    // Update arc with new value
    lv_arc_set_value(objects.arc_shadow_bg, value);

    //Set image rotation so needle moves with the ARC
    lv_image_set_rotation(objects.img_hovering_needle, value * 10);

    // Update label values
    lv_label_set_text_fmt(objects.label_milage_bg, "%d", value);
    lv_label_set_text_fmt(objects.label_milage_fg, "%d", value);
  }
  //LV_LOG_USER("ARC value updated");
}
...
void loop() {
  ...
  */
  ...
  //Set ARC value and fire a value changed event
