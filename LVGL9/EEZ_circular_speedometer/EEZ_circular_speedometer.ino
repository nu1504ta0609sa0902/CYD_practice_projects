...
uint8_t value = 0;
uint8_t maxValue = 240;
uint8_t maxRandomValue = 10;
void mockArcSpeed() {

  // uint8_t change = 1;
  //Add randomness to indicator/arc/scale
  uint8_t change = random(0, maxRandomValue);
  if (change % 3 == 0 || value < maxRandomValue)
    isDirectionUp = true;
  else if (value > (maxValue - maxRandomValue))
    isDirectionUp = false;
...
void setup() {
  ...
  lv_obj_add_event_cb(objects.arc_speedometer, value_changed_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
}
...
static void value_changed_event_cb(lv_event_t *e) {
  ...
  if (code == LV_EVENT_VALUE_CHANGED) {
    //Update arc and label
    lv_arc_set_value(objects.arc_speedometer, value);
    lv_arc_set_value(objects.arc_behind_scale, value);

    //Update speed label
    lv_label_set_text_fmt(objects.label_speed_kmh, "%d", value);
    lv_label_set_text_fmt(objects.label_speed_kmh_1, "%d", value);
  }
}
...
void loop() {
  ...
  */
  ...
  //Manually trigger event for ARC
  lv_obj_send_event(objects.arc_speedometer, LV_EVENT_VALUE_CHANGED, NULL);
